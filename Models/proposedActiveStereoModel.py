...
if __name__ == "__main__":
	...
	for index, point in enumerate(car_content):
        ...
        # Repeat the process for the other dot
    ...
    # Add extra dots for each set of dots
	realWorldPoint = np.repeat(realWorldPoint, 2, axis=1)
	realWorldPoint[0,1::2] += dotTranslation[0]
	realWorldPoint[1,1::2] += dotTranslation[1]
	...
	for a in range(len(l_image_point)):
		...
		NphotonsForEachPixel, xPx, yPx = pixelization(d = r, aux_x = imagePlanePoint[0]/pxWidth, aux_y = imagePlanePoint[1]/pxWidth)
	...
	plt.subplots()
	# plt.imshow(NphotonsMatrix[::-1,:] + 1, cmap='gray')
	plt.imshow(NphotonsMatrix[-170-4:-210-4:-1,3050:3100], cmap='gray')
	...
	plt.subplot(2,1,1)
	plt.imshow(imgOut[-170-4:-210-4:-1,3050:3100], cmap='jet')
	...
	plt.subplot(2,1,2)
	plt.imshow(imgOut[-170-4:-210-4:-1,3050:3100] > 200, cmap='jet')
