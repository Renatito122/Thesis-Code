    ...
    for frame in sorted(os.listdir("velodyne_1")):
        ...
        for point in content:
            ...
            if count == 0:
                ...
                # Calculate centroid
                centroid_x1 = np.sum(XPx * image1) / np.sum(image1)
                centroid_y1 = np.sum(YPx * image1) / np.sum(image1)
            ...
            if count == 0:
                ...
                # Calculate centroid
                centroid_x2 = np.sum(XPx * image2) / np.sum(image2)
                centroid_y2 = np.sum(YPx * image2) / np.sum(image2)
